...
- [ ] **`private-vm-ssh` + tmux integration** — sessionizer pattern: a `~/sessions/private-notes/` stub triggers `private-vm-start`, opens a tmux session named `private-notes`, default-command is `private-vm-ssh`. SSH `ControlPersist` keeps subsequent panes cheap.
